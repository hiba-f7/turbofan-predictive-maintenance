...
def generate_pdf_report(engine_id, cycle, pred_rul, safe_floor, status_text, diagnosis, margin_calibrated):
    ...
    pdf.cell(0, 10, f"Engine {engine_id} - Maintenance Summary", ln=True)

    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 6, f"Generated by Turbofan RUL Predictor dashboard", ln=True)
    pdf.ln(4)

    pdf.set_font("Helvetica", "", 11)
    pdf.cell(0, 8, f"Current cycle: {cycle}", ln=True)
    pdf.cell(0, 8, f"Predicted Remaining Useful Life: {pred_rul:.0f} cycles", ln=True)
    margin_note = "" if margin_calibrated else " (default, uncalibrated margin)"
    pdf.cell(0, 8, f"Safe operating floor (95% confidence){margin_note}: {safe_floor:.0f} cycles", ln=True)
    pdf.cell(0, 8, f"Status: {status_text}", ln=True)

    pdf.ln(4)
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 8, "Suggested inspection points:", ln=True)
    pdf.set_font("Helvetica", "", 11)
    if diagnosis:
        for d in diagnosis:
            pdf.multi_cell(0, 6, f"- {d['subsystem']}: {d['action']} (indicator: {d['sensor_name']})")
    else:
        pdf.cell(0, 8, "No dominant risk driver identified at this cycle.", ln=True)
